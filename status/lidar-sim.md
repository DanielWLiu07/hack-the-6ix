...
## [MILESTONE] Real RPLIDAR C1 live end-to-end on hardware (with human, on-site)
Got the physical lidar working with the human at the venue. Key discoveries and fixes:
- Hardware is a SLAMTEC RPLIDAR C1 (model 0x41), NOT WiFi (that was a misdiagnosis). It is TTL UART, wired to the Pi GPIO serial pins, not USB. So it is on /dev/ttyAMA0 (Pi 5 GPIO UART), NOT /dev/serial0 (which points at ttyAMA10) and NOT USB. Baud 460800.
- The C1 stays silent until commanded, and its motor does not spin on power alone: you must send SET_MOTOR_PWM (0xA5 0xF0, value ~660), wait ~2s to spin up, then START_SCAN. Then full 360deg data streams (~3600 pts/3s, full angular coverage).
- Pi access: SSH pi@10.186.57.69 (pw pi), key installed at ~/.ssh/ht6_pi on the Mac. Pi 5, hostname rpi. Pi reaches the laptop hub at http://10.186.57.117:3001 (no hotspot client isolation on "Shiba 8").
Wrote a self-contained Pi node `~/c1_slam_node.py`: reads+parses the C1 (resyncing packet parser), runs scan-to-map ICP (scan_match.ScanMapper) + occupancy grid (occupancy.OccupancyGrid, 128x128 @0.06m), and emits lidar_scan + slam_pose + slam_map as role=robot. Copied scan_match.py + occupancy.py to /home/pi. Installed on Pi: python3-serial, python3-socketio, python3-numpy.
Persistence: systemd unit `c1-slam.service` (enabled, Restart=always, starts on boot). `sudo systemctl status/restart c1-slam`.
VERIFIED end-to-end: a ui client on the hub receives lidar_scan (real room, 150-370 pts), slam_pose, AND slam_map - so @server-core HAS whitelisted slam_pose/slam_map in the relay (confirmed live). Clean rectangular room map produced when the space was clear.
Notes / next: motor at PWM 660 spins ~2-6 Hz (below the C1's 10 Hz rating) so scans are a bit sparse; could raise PWM for denser scans (1023 gave no data, needs tuning). SERVER_URL is hardcoded to the laptop's current DHCP IP (10.186.57.117) in the unit - update if it changes. @lidar-pi: this Pi node is a new GPIO+serial-motor path distinct from your USB lidar_node; it should live in robot/lidar/pi/ - coordinate to fold it in (it is only on the Pi + my scratch right now, not committed).
NAVIGATION reality: SLAM (localization + live occupancy map) works on real hardware. Autonomous navigation (path planning + drive control) is a separate layer not built; the rover is teleop per the plan. SLAM is the map/localization foundation, not an autonomous driver.

...
**On-device FPS** (`robot/vision/bench.py`, re-run on the board via
`./deploy_unoq.sh bench`). Laptop reference (arm64), HSV path — huge headroom, so
the board stays camera-bound not compute-bound:
...
Detector accuracy (synthetic ground truth, 200 frames): **precision 0.965 /
recall 0.891**. The headline YOLOv8n-int8 on-board FPS drops in here once
measured on the UNO Q — the harness is ready and quoted verbatim.

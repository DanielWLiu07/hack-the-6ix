...
- [ ] Servos are on their **own 5 V ≥5 A buck** with the **1000 µF cap** across
      the rail near the PCA9685 `V+`. PCA9685 `VCC` (logic) ← UNO Q **3.3 V**.
      **Never** jumper `VCC` to `V+` (see PINOUT.md gotcha).
...
- [ ] Move ONE joint a small delta, watch the servo-rail voltage (§2) for sag:
      `S 90 100 90 90 90 1000` (shoulder → 100°, 1 s). Motion must be **smooth**,
...
- [ ] `Q` shows a plausible `<ultra_cm>` for a target ~30 cm away.
- [ ] Put a hand **< 15 cm** → state flips to `1` (OBSTACLE); forward drive is
      reflex-zeroed. Pull back **> 25 cm** → auto-clears to `0` (hysteresis).
...
2. **Bad subsystem, rest OK?** Keep teleop/vision running and just don't command
   the broken joint. `OBSTACLE`/`WATCHDOG`/`ESTOP` all fail safe by design.

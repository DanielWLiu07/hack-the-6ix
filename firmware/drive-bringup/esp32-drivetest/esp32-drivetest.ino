...
//
// Wiring:
//   LEFT  driver: RPWM=GPIO25  LPWM=GPIO26
//   RIGHT driver: RPWM=GPIO32  LPWM=GPIO33
//   Both drivers: R_EN & L_EN tied to 3V3 (always enabled)
//   Driver VCC -> 3V3, GND -> common ground, B+ -> +11.1V, B- -> supply -
//
// Motors only physically move when the battery is ON. Flash with the battery
// OFF first (sequence runs, nothing spins) to confirm it works, then power up.

const int L_RPWM = 25, L_LPWM = 26;   // LEFT motor
const int R_RPWM = 32, R_LPWM = 33;   // RIGHT motor
...
void runSequence() {
  ...
  drive(0, 0);
  Serial.println("DONE - stopped. Press EN to re-run, or send: g=sequence "

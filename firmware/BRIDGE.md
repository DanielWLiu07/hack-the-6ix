...
| Joint angles `joints[4]` | int | degrees, 0–180. Order: `[base, shoulder, elbow, gripper]` (direct GPIO servos D3/D9/D10/D11). |
...
Priority: `ESTOP` > `WATCHDOG` > `OK`.

> State **1 (`OBSTACLE`)** was the ultrasonic reflex. The HC-SR04 was removed in
> the 4-servo redesign, so state 1 is **retired and reserved** — the remaining
> numbers stay put (`OK`=0, `WATCHDOG`=2, `ESTOP`=3) so fw-linux/web code that
> keys on "state 3 = ESTOP" keeps working.

## 3. RPC surface (Linux -> MCU)

| Call | Args | Returns | Notes |
|---|---|---|---|
| `set_drive(l, r)` | float, float | int state (see §2) | Ignored (returns state) unless state is `OK`. |
| `move_servos(joints, duration_ms)` | int[4], int | int state | Starts interpolated move. Ignored in `ESTOP`/`WATCHDOG`. |
...
| `get_status()` | - | int[8] (below) | Poll at 5 Hz for telemetry; don't poll faster than 20 Hz. |
| `zero_all()` | - | int state | Interpolated move of all 4 servos to 90° over 1500 ms. For assembly/horn alignment. |

`get_status()` return, fixed order:

```
[ state, battery_mv, j0, j1, j2, j3, drive_l_pct, drive_r_pct ]
   §2      mV (0=n/a)   current interpolated   −100...100 int
                        joint degrees (int)      (l/r × 100)
```

fw-linux maps this straight into the root-schema `telemetry` event
(`arm` = j0...j3, `drive.l/r` = pct/100, `state` string comes from fw-linux's own
task state machine, except MCU state 3 -> `"ESTOP"` always wins).

**No MCU->Linux push calls in v1.** Linux learns everything by polling
`get_status()` at 5 Hz. (Watchdog protection is already local to the MCU, so
push adds no safety; polling keeps both sides simpler.)
...
void setup() {
  ...
  Bridge.provide_safe("move_servos", move_servos);    // (int×4, int)->int
  ...
  Bridge.provide_safe("get_status", get_status);      // ()->MsgPack::arr_t<int>[8]
...
**Wire signatures**: `move_servos` takes **5 flat int args**
`(j0,j1,j2,j3,duration_ms)` - flat scalars marshal trivially on both
sides; conceptually it's still `joints[4] + duration_ms`. `get_status`
returns an 8-element MsgPack int array (`MsgPack::arr_t<int>`), order per §3.
...
S <j0> <j1> <j2> <j3> <ms>       move_servos    e.g.  S 90 45 120 30 1000
H                              heartbeat
E                              estop
C                              clear_estop
Q                              get_status
...
- `Q` -> `ST <state> <battery_mv> <j0> <j1> <j2> <j3> <l_pct> <r_pct>`
